                             ...
                             LoggerServiceBase logger) : IQuestionService
{
    ...
    {
        QuestionStatus[] status = [QuestionStatus.SendAgain];
    ...
    public async Task<string> AddQuiz(AddQuizModel model, CancellationToken cancellationToken)
    {
        ...
        var responses = await questionApi.GetSimilarQuestionsForQuiz(new()
        {
        ...
        _ = await notificationService.PushNotificationByUserId(Strings.DynamicLessonTestPrepared.Format(lessonName), Strings.DynamicLessonTestPreparedForYou.Format(lessonName, result), model.UserId);
    ...
    public async Task<bool> AddQuiz(bool timePass = false, CancellationToken cancellationToken = default)
    {
        try
        {
            AppStatics.SenderTryQuestion = false;
        ...
        {
            AppStatics.SenderTryQuestion = true;
        }
    }
