...
public class RTeacherClassRoomConfiguration : IEntityTypeConfiguration<RTeacherClassRoom>
{
    public void Configure(EntityTypeBuilder<RTeacherClassRoom> builder)
    {
        builder.ToTable("RTeacherClassRooms");
        ...
        builder.HasOne(d => d.Teacher).WithMany(p => p.RTeacherClassRooms).HasForeignKey(d => d.TeacherId).HasPrincipalKey(x => x.Id);
