...
namespace Business.Features.Questions.Commands.Quizs;
                                   ...
                                   UserRules userRules) : IRequestHandler<AddQuizCommand, GetQuizModel>
{
    public async Task<GetQuizModel> Handle(AddQuizCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await quizDal.AddAsync(quiz);
            await quizQuestionDal.AddRangeAsync(questions);
        ...
        var added = await quizDal.GetAsyncAutoMapper<GetQuizModel>(
