...
public sealed class GetBookPageImageQuery : IRequest<MemoryStream?>, ISecuredRequest<UserTypes>
{
...
public sealed class GetBookPageImageQueryHandler(IBookDal bookDal,
                                                 ICommonService commonService) : IRequestHandler<GetBookPageImageQuery, MemoryStream?>
{
    public async Task<MemoryStream?> Handle(GetBookPageImageQuery request, CancellationToken cancellationToken)
    {
        ...
        var filePath = Path.Combine(AppOptions.BookFolderPath, $"{request.BookId}", $"{request.Page}{request.Extension}");
