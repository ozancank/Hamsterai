...
public class RTeacherLessonConfiguration : IEntityTypeConfiguration<RTeacherLesson>
{
    public void Configure(EntityTypeBuilder<RTeacherLesson> builder)
    {
        builder.ToTable("RTeacherLessons");
        ...
        builder.HasOne(d => d.Teacher).WithMany(p => p.RTeacherLessons).HasForeignKey(d => d.TeacherId).HasPrincipalKey(x => x.Id);
